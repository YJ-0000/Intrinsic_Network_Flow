...
mkdir('G_ICA_DMD_video_HCP_REST_fbDMD_REST1_exact_mode_image');
save_dir = [pwd filesep 'G_ICA_DMD_video_HCP_REST_fbDMD_REST1_exact_mode_image'];
...
for pair_num = 1:4%size(Phi_orig_complex,2)/2-1
    ...
    %         plot_subcortex(cifti_data, [], [], min_scale, max_scale, 'thalamus')
            ...
            % 프레임 캡처 후 동영상에 기록
            fname = fullfile(save_dir_dm, sprintf('DM%02d_map_%s_%03d.jpg', pair_num, 'cortex',frame));
    ...
    if pair_num == 1
        structure_label_list = {'striatum','thalamus','cerebellum','cerebellum_flat'};
    else
        structure_label_list = {'cerebellum','cerebellum_flat'};
    end
    ...
    for i_struc = 1:length(structure_label_list)
        structure_label = structure_label_list{i_struc};
         ...
         for frame = 1:frame_length
            ...
            fname = fullfile(save_dir_dm, sprintf('DM%02d_map_%s_%03d.jpg', pair_num, structure_label,frame));
            print(fig, fname, '-djpeg', '-r300');
         end
    end
